...
本项目侧边栏和路由是绑定在一起的，所以你只有在 `@/router/router.js` 下面配置对应的路由，侧边栏就能动态的生成了。大大减轻了手动编辑侧边栏的工作量。当然这样就需要在配置路由的时候遵循很多的约定。

## 配置项
首先我们了解一些本项目配置路由时提供了哪些配置项。

```js
//当设置 true 的时候该路由不会在侧边栏出现 如401，login等页面(默认 false)
hidden: true

//当设置 noredirect 的时候该路由不会在面包屑导航中出现
redirect: noredirect

//当设置 true 的时候永远会显示根菜单，不设置的情况下只有当子路由个数大于 1 个时才会显示根菜单
//当你一个路由下面的 children 声明的路由大于 1 个时，自动会变成嵌套的模式。只有 1 个时会将那个子路由当做根路由
alwaysShow: true
...
这里的路由分为三种，`constantRouterMap`、`asyncRouterMap`和其他不展示在Menu中的页面。

**constantRouterMap** 代表那些不需要动态判断权限的路由，如登录页，404，等通用页面。

**asyncRouterMap** 代表那些需要动态判断权限并通过 `addRouters` 动态添加的页面。具体的会在 [权限判断](https://panjiachen.github.io/vue-element-admin-site/#/permission) 页面介绍。

> 这里所有的组件使用自定义方法 `_import ` 引入，具体介绍见[路由懒加载]

其它的配置和 [vue-router](https://router.vuejs.org/zh-cn/) 官方并没有区别，自行查看文档。

?> **注意事项：**这里有一个需要非常注意的地方就是：404页面一定要最后加载，如果放在 constantRouterMap 一同声明了404，后面的所有页面都会被拦截到404，详细的问题见 [addRoutes when you've got a wildcard route for 404s does not work](https://github.com/vuejs/vue-router/issues/1176)
...
> 代码地址: [@/views/layout/components/Sidebar](http://10.21.20.76:6868/xiyt/EF-UI/blob/master/src/modules/layout/Sidebar)

这里同时也改造了 `element-ui` 默认侧边栏不少的样式，所有的css都可以在 [@/styles/sidebar.scss](http://10.21.20.76:6868/xiyt/EF-UI/blob/master/src/styles/sidebar.scss) 中找到，可以根据自己的需求进行修改。
...
> 组件地址: [@/components/Breadcrumb](http://10.21.20.76:6868/xiyt/EF-UI/blob/master/src/components/Breadcrumb/index.vue)
...
> 代码地址: [@/components/ScrollPane](http://10.21.20.76:6868/xiyt/EF-UI/blob/master/src/components/ScrollPane/index.vue)
